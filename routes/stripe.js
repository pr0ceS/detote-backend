...
const createInvoiceAndOrder = async (customer, data, lineItems) => {
    ...
    // Pinterest tracking
    try {
      const url = "https://api.pinterest.com/v5/ad_accounts/549767078078/events";
      const headers = {
        'Authorization': 'Bearer pina_AIA2RFAWAA67AAQAGAAEQDGZUNS3LDABAAAABOULSLJUVJUI5KWVB64UMI2WQWZNI7BCJ3Y7OVNWOBYVX54KYFMIJHO2J5QA',
      ...
      const totalQuantity = await lineItems.data.reduce((total, lineItem) => {
        return total ? total + lineItem.quantity : lineItem.quantity;
      }, 0);
      const products = await Promise.all(await newOrder.products[0].data.map(async (item) => {
      ...
      const body = {
        "data": [{
          ...
          "user_data": {
            "em": [hashNormalizedEmail(await data?.customer_details?.email)],
            "external_id": [hashNormalizedEmail(await customer?.metadata?.userId ? await customer?.metadata?.userId !== "GUEST" ? await customer?.metadata?.userId : await customer?.metadata?.visitRef : await customer?.metadata?.visitRef)]
            // Include other user_data fields
          },
          "custom_data": {
            "currency": "EUR",
            "value": (await data?.amount_total / 100).toFixed(2),
            "content_ids": products,
            "num_items": await totalQuantity,
          },
        }]
      };
